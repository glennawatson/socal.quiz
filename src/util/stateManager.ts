...
export class StateManager {
    private static tableName: string = "QuizStateTable";
    private client: TableClient;

    public constructor(connectionString?: string | undefined, client?: TableClient) {
        if (!client) {
            if (!connectionString) {
                throw new Error("Must have a valid connection string");
            }

            this.client = TableClient.fromConnectionString(connectionString, StateManager.tableName);
        } else {
            this.client = client;
        }
    }

    public async getState(guildId: string, channelId: string): Promise<QuizState> {
        const entity = await this.client.getEntity<TableEntity<QuizState>>(guildId, channelId);
