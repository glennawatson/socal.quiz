...
export class GuildStorage {
    ...
    constructor(connectionString?: string) {
        ...
        this.guildClient = TableClient.fromConnectionString(connectionString, 'GuildRegistrations');
    }

    public async isGuildRegistered(guildId: string): Promise<boolean> {
        try {
            const entity = await this.guildClient.getEntity("RegisteredGuilds", guildId);
            return !!entity;
        } catch (error) {
