...
export async function sendQuestionSummary(rest: REST, imageStorage: QuizImageStorage, question: Question, quiz: QuizState, questionNumber: number) {
    const correctAnswer = question.answers.find(x => x.answerId === question.correctAnswerId);
    ...
    if (question.explanationImagePartitionKey) {
        const imageUrl =
            await imageStorage.getExplanationImagePresignedUrl(
                question.bankName,
                question.questionId,
            );
