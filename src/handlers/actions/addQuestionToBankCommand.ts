...
export class AddQuestionToBankCommand implements IModalHandlerCommand {
  ...
  public async execute(
    ...
    modal.addComponents(
      ...
      new ActionRowBuilder<TextInputBuilder>().addComponents(
        this.createTextInput(
          this.componentIds.questionText,
          "Question Text",
      ...
      new ActionRowBuilder<TextInputBuilder>().addComponents(
        this.createTextInput(
          this.componentIds.timeoutTimeSeconds,
      ...
      new ActionRowBuilder<TextInputBuilder>().addComponents(
        this.createTextInput(
          this.componentIds.explanationImageUrl,
    ...
    modal.addComponents(
      new ActionRowBuilder<TextInputBuilder>().addComponents(
        this.createTextInput(
          this.componentIds.correctAnswerIndex,
  ...
  public async handleModalSubmit(
    ...
    const getComponentValue = (customId: string) =>
      components?.find((row) => row.components?.[0]?.custom_id === customId)
        ?.components?.[0]?.value;
    const getComponentValueNumber = (customId: string): number | undefined => {
    ...
    const questionShowTimeMs =
      (getComponentValueNumber(this.componentIds.timeoutTimeSeconds) ?? 20) *
      1000;
