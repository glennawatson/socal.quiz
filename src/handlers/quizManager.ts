...
import {APIInteraction, APIInteractionResponse, ButtonStyle, InteractionType, Routes,} from "discord-api-types/v10";
...
export class QuizManager {
    ...
    constructor(private readonly rest: REST, private readonly quizStateStorage: QuestionStorage, public readonly summaryDurationMs: number = 5000) {
    ...
    public async startQuizInternal(
        ...
        this.stopQuiz(channelId);
        return createEphemeralResponse(`Quiz for channel ${quiz.channelId} completed successfully.`);
    }
    ...
    public async handleAnswer(
        ...
        console.debug(`now conducting handling for quiz in ${interaction.channel.id}`);

        const quiz = this.quizzes.get(interaction.channel.id);
        if (!quiz) {
            console.error(`no quiz found for channel ${interaction.channel.id} while handling the message reply with quizzes having ${this.quizzes.size} entries`);
