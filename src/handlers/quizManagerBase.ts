...
import {createEphemeralResponse, isNullOrWhitespace} from "../util/interactionHelpers.js";
...
export abstract class QuizManagerBase {
     ...
     */
    public abstract nextQuizQuestion(guildId: string, channelId: string): Promise<void>;
    ...
    public async startQuiz(
        guildId: string,
        channelId: string,
        questionBankName: string): Promise<APIInteractionResponse> {
    ...
    public async startQuizInternal(
        questions: Question[],
        guildId: string,
        channelId: string): Promise<APIInteractionResponse> {
    ...
    public async handleAnswerInteraction(
        ...
        const [_, interactionId, selectedAnswerId] = interaction.data.custom_id.split("_");
        ...
        return await this.answerInteraction(interaction.guild_id, interaction.channel.id, interaction.user.id, selectedAnswerId);
    }
     ...
     * @returns {Promise<APIInteractionResponse>} - A promise that resolves to an interaction response.
     */
    public abstract answerInteraction(guildId: string, channelId: string, userId: string, selectedAnswerId: string): Promise<APIInteractionResponse>;
}