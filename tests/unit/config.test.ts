...
describe('Config', () => {
    ...
    it('should initialize with provided values', async () => {
        ...
        const mockDiscordBotService = new DiscordBotService(mockToken, mockClientId, mockGuildStorage, mockQuestionStorage);

        await Config.initialize(mockToken, mockClientId, mockPublicKey, mockQuestionStorage, mockGuildStorage, mockDiscordBotService);
    ...
    it('should throw error if DISCORD_BOT_TOKEN environment variable is missing', async () => {
        ...
        await expect(Config.initialize()).rejects.toThrow('Environment variable DISCORD_BOT_TOKEN is missing.');
    ...
    it('should throw error if DISCORD_CLIENT_ID environment variable is missing', async () => {
        ...
        await expect(Config.initialize()).rejects.toThrow('Environment variable DISCORD_CLIENT_ID is missing.');
    ...
    it('should throw error if DISCORD_PUBLIC_KEY environment variable is missing', async () => {
        ...
        await expect(Config.initialize()).rejects.toThrow('Environment variable DISCORD_PUBLIC_KEY is missing.');
    ...
    it('should initialize discordBotService with new instance when not provided', async () => {
        ...
        await Config.initialize(mockToken, mockClientId, mockPublicKey, mockQuestionStorage, mockGuildStorage);

        expect(Config.discordBotService).toBeInstanceOf(DiscordBotService);
        expect(DiscordBotService).toHaveBeenCalledWith(mockToken, mockClientId, mockGuildStorage, mockQuestionStorage);
    });
});
