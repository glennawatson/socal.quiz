...
describe("QuizManagerBase", () => {
    ...
    describe("startQuiz", () => {
        it("should return an error if question bank name is null", async () => {
            const response = await quizManagerBase.startQuiz("guild123", "channel123", null as any);
            expect(response).toEqual(createEphemeralResponse("There is no valid question bank name"));
        });

        it("should return an error if question bank name is empty", async () => {
            const response = await quizManagerBase.startQuiz("guild123", "channel123", "");
            expect(response).toEqual(createEphemeralResponse("There is no valid question bank name"));
        });

        it("should return an error if no questions are found", async () => {
            quizStateStorageMock.getQuestions = vi.fn().mockResolvedValue([]);
            const response = await quizManagerBase.startQuiz("guild123", "channel123", "bank123");
            expect(response).toEqual(createEphemeralResponse("There are no valid questions in the question bank bank123"));
        });
        ...
        it("should call startQuizInternal with valid questions", async () => {
            ...
            const startQuizInternalSpy = vi.spyOn(quizManagerBase, "startQuizInternal");

            await quizManagerBase.startQuiz("guild123", "channel123", "bank1");

            expect(startQuizInternalSpy).toHaveBeenCalledWith(questions, "guild123", "channel123");
        });
    });

    describe("startQuizInternal", () => {
        it("should return an error if there are no valid questions", async () => {
            const response = await quizManagerBase.startQuizInternal([], "guild123", "channel123");
        ...
        it("should return an error if there are invalid questions", async () => {
            ...
            const response = await quizManagerBase.startQuizInternal(questions, "guild123", "channel123");
            expect(response).toEqual(createEphemeralResponse("There are invalid questions with IDs: q1"));
        });
        ...
        it("should call stopQuiz and runQuiz with valid questions", async () => {
            ...
            await quizManagerBase.startQuizInternal(questions, "guild123", "channel123");

            expect(stopQuizSpy).toHaveBeenCalledWith("guild123", "channel123");
            expect(runQuizSpy).toHaveBeenCalledWith(expect.objectContaining({
    ...
    describe("handleAnswerInteraction", () => {
        it("should return an error if interaction ID is missing", async () => {
            const interaction: APIInteraction = {
                ...
                token: "token123",
                version: 1,
                data: { id: "data1", type: 1, name: "name1", custom_id: "quiz__answer123" },
                user: { id: "user123", username: "user", discriminator: "0001" },
            } as any;

            const response = await quizManagerBase.handleAnswerInteraction(interaction);
            expect(response).toEqual(createEphemeralResponse("Could not find a valid interaction id"));
        });
        ...
        it("should return an error if guild_id is missing", async () => {
            ...
            const response = await quizManagerBase.handleAnswerInteraction(interaction);
            expect(response).toEqual(createEphemeralResponse("Must have a valid guild id."));
        });
        ...
        it("should return an error if channel id is missing", async () => {
            ...
            const response = await quizManagerBase.handleAnswerInteraction(interaction);
            expect(response).toEqual(createEphemeralResponse("Must have a valid channel"));
        });
        ...
        it("should return an error if interaction ID is missing", async () => {
            ...
            const response = await quizManagerBase.handleAnswerInteraction(interaction);
            expect(response).toEqual(createEphemeralResponse("Could not find a valid answer response"));
        });
        ...
        it("should return an error if selected answer ID is missing", async () => {
            const interaction: APIChatInputApplicationCommandInteraction = {
                ...
                token: "token123",
                version: 1,
                data: { id: "data1", type: 1, name: "name1", component_type: ComponentType.Button, custom_id: "quiz_" },
        ...
        it("should return an error if user ID is missing", async () => {
            const interaction: APIChatInputApplicationCommandInteraction = {
                ...
                token: "token123",
                version: 1,
                data: { id: "data1", type: 1, name: "name1", component_type: ComponentType.Button, custom_id: "quiz_123_answer123" },
                user: undefined,
            } as any as APIChatInputApplicationCommandInteraction;

            const response = await quizManagerBase.handleAnswerInteraction(interaction);
            expect(response).toEqual(createEphemeralResponse("Could not find a valid user id"));
        });
        ...
        it("should call answerInteraction with correct parameters", async () => {
            const interaction: APIChatInputApplicationCommandInteraction = {
                ...
                token: "token123",
                version: 1,
                data: { id: "data1", type: 1, name: "name1", custom_id: "quiz_123_answer123" },
                user: { id: "user123", username: "user", discriminator: "0001" },
            } as any;

            const answerInteractionSpy = vi.spyOn(quizManagerBase, "answerInteraction");

            await quizManagerBase.handleAnswerInteraction(interaction);

            expect(answerInteractionSpy).toHaveBeenCalledWith("guild123", "channel123", "user123", "answer123");
        });
    });
});