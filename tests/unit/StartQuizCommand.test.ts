    ...
    describe('execute', () => {
        it('should start the quiz and return a confirmation message', async () => {
            const quizManagerMock = {
                startQuiz: vi.fn().mockResolvedValue(createEphemeralResponse('Quiz started')),
            };
            ...
            expect(quizManagerMock.startQuiz).toHaveBeenCalledWith('channel-id', 'sampleBank');
        ...
        it('should return an error if the guild id or bank name is missing', async () => {
            const response = await startQuizCommand.execute(generateBankOptions('', ''));

            expect(response).toEqual(generateOptionMissingErrorResponse('guild_id'));

            const response2 = await startQuizCommand.execute(generateBankOptions('123', ''));
