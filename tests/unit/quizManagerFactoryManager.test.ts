...
import {QuizManagerFactory, QuizManagerFactoryManager} from "../../src/handlers/quizManagerFactoryManager.js";
...
describe("QuizManagerFactoryManager", () => {
    ...
    beforeEach(() => {
        ...
        quizManagerFactoryManager = new QuizManagerFactoryManager(quizManagerFactory);
    });
    ...
    describe("getQuizManager", () => {
        ...
        it("should throw an error if the manager cannot be created", async () => {
            ...
            await expect(quizManagerFactoryManager.getQuizManager(guildId)).rejects.toThrowError(`could not find a quiz manager for guild ${guildId}`);
        });
    });
});