...
import {createEphemeralResponse, generateErrorResponse} from "../../src/util/interactionHelpers";
...
describe('DiscordBotService', () => {
    ...
    describe('start', () => {
        it('should register default commands and mark the guild as registered', async () => {
            ...
            expect(commandManagerMock.registerDefaultCommands).toHaveBeenCalledWith('guild-id');
            expect(guildStorageMock.markGuildAsRegistered).toHaveBeenCalledWith('guild-id');
        });
    });
    ...
    describe('getQuizManager', () => {
        it('should return an existing QuizManager', async () => {
            ...
            discordBotService['quizManagers'].set(guildId, Promise.resolve(quizManagerMock));
    ...
    describe('handleInteraction', () => {
        it('should handle interactions outside a guild with an ephemeral response', async () => {
            ...
            expect(response).toEqual(createEphemeralResponse('This interaction must be performed within a guild.'));
        });
        ...
        it('should delegate answer interactions to the QuizManager', async () => {
            ...
            quizManagerMock.handleAnswer = vi.fn().mockResolvedValue(createEphemeralResponse('Answer handled'));

            discordBotService['quizManagers'].set('guild-id', Promise.resolve(quizManagerMock));
        ...
        it('should delegate other interactions to the CommandManager', async () => {
            ...
            guildStorageMock.isGuildRegistered = vi.fn().mockResolvedValue(true);
            commandManagerMock.handleInteraction = vi.fn().mockResolvedValue(createEphemeralResponse('Command handled'));
