...
internal readonly struct InternalAccessTokenResult(AccessTokenResultStatus status, AccessToken token)
{
    [JsonConverter(typeof(JsonStringEnumConverter<AccessTokenResultStatus>))]
    public AccessTokenResultStatus Status { get; init; } = status;

    public AccessToken Token { get; init; } = token;
}
